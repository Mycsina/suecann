...
A connection from $i$ to $j$ with sign $s$ contributes $s \cdot W \cdot
\text{out}_i$ to $j$'s aggregation. At inference the network is evaluated at
all six sweep weights $\{-2.0, -1.0, -0.5, 0.5, 1.0, 2.0\}$; the argmax output
is averaged across sweeps before intent selection.
