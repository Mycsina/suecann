...
def test_lexicographic_pareto_ranking():
    from src.wann.population import _pareto_rank

    # Case 1: Same Pareto front level, different fitnesses.
    # A: fitness 12.0, complexity 5 (simplicity 0)
    # B: fitness 10.0, complexity 2 (simplicity 3)
    # Neither dominates. Both are Level 0.
    # Because A has higher fitness, under Lexicographic selection, A should get a higher score.
    fitnesses = [12.0, 10.0]
    complexities = [5, 2]
    scores = _pareto_rank(fitnesses, complexities)
    assert scores[0] > scores[1]

    # Case 2: Dominance (Level 0 vs Level 1).
    # A: (8.0, 5) | B: (10.0, 2) -> B dominates A on both fitness and simplicity.
    # So B is Level 0, A is Level 1.
    # B's score must be strictly greater than A's score.
    fitnesses = [8.0, 10.0]
    complexities = [5, 2]
    scores = _pareto_rank(fitnesses, complexities)
    assert scores[1] > scores[0]

    # Case 3: Identical fitnesses, different complexities.
    # A: (10.0, 5) | B: (10.0, 2) -> B dominates A on simplicity.
    # B's score must be strictly greater than A's score.
    fitnesses = [10.0, 10.0]
    complexities = [5, 2]
    scores = _pareto_rank(fitnesses, complexities)
