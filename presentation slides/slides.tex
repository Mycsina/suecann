...
% ======================================================================
% SLIDE 5a — The Elite heuristic (context for the "Elite + dials" intents)
% ======================================================================
\begin{frame}{The Elite heuristic is a strong, partner-aware rule base --- the high floor every intent builds on}
  \begin{columns}[T]
    \column{0.33\textwidth}
      \centering
      \begin{block}{\centering\textbf{When leading}}
        \vspace{1em}
        \footnotesize
        Cash a \textbf{guaranteed master} (a sure winner) first.\\[4pt]
        Else \textbf{probe}: lead the \textbf{lowest} card of your \textbf{longest} side-suit.\\[4pt]
        \textit{``Bank what's safe, prod with what's cheap.''}
      \end{block}
    \column{0.33\textwidth}
      \centering
      \begin{block}{\centering\textbf{When following}}
        \vspace{1em}
        \footnotesize
        Take the trick with your \textbf{cheapest} winning card.\\[4pt]
        Can't win? Throw your \textbf{lowest} --- never overspend.\\[4pt]
        \textit{``Win cheap, lose cheap.''}
      \end{block}
    \column{0.33\textwidth}
      \centering
      \begin{block}{\centering\textbf{When void}}
        \vspace{1em}
        \footnotesize
        Cut with the \textbf{cheapest} trump that wins; \textbf{hold high trumps back}.\\[4pt]
        Partner safe or no trump? \textbf{Don't spend one}.\\[4pt]
        \textit{``Trump just enough.''}
      \end{block}
  \end{columns}
  \vspace{8pt}
  \centering
  \footnotesize
  Throughout, \textbf{play for the partnership}: \emph{smear} points onto a secure partner trick, raise your \textbf{highest} to protect an unsecured one.\\
  Expert cooperative play with \textbf{no search} --- so realistic headroom above it is only $\sim$\textbf{8 game-points}.
\end{frame}
...
\begin{frame}{Phase 1: co-evolutionary self-play with dynamic per-decision brain routing}
  ...
  \vspace{2pt}
  {\footnotesize Dynamic per-decision routing via \texttt{Am\_I\_Leading} flag.
