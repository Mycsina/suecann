...
\begin{frame}{Three root-cause fixes turned a collapsed network into an Elite-beating champion}
  ...
  \begin{columns}[T]
    ...
    \column{0.33\textwidth}
      ...
      (29 gates, 49 conns)\\
      {\footnotesize\emph{equal strength}}
    \end{columns}
  \vspace{6pt}
  \centering
\end{frame}
...
% ======================================================================
\begin{frame}{The styled resolver enabled the win but also sets the ceiling: we measured four strength levels}
  \centering
  \begin{tabular}{@{}lcccc@{}}
    \toprule
  ...
  \footnotesize
  \textbf{Pure intent:} each of the 3 intents played alone (no WANN) \\ \hspace{14pt}$\rightarrow$ $\approx$Elite — the ``raised floor'' that prevents collapse.\\
  \textbf{Oracle:} perfect intent selection at every decision \\ \hspace{14pt}$\rightarrow$ 58\% is the \textbf{absolute cap} of the current vocabulary.\\
  \textbf{RF = Random Forest} trained on belief features to predict the oracle's choice. \\ \hspace{14pt}63--71\% accuracy shows the \textbf{features carry usable signal} the WANN has not yet learned.\\[4pt]
  \textbf{The +2\% gap} (47\% floor $\rightarrow$ 52\% champion) \textbf{is the learned deviation overlay.}
  ...
  \begin{columns}[T]
    \column{0.50\textwidth}
      ...
      \begin{itemize}
        \item 3 intents nearly equivalent in fitness $\rightarrow$ weak learning signal
        \item Lead decision harder: RF gap $+$2.5 pp (vs.\ follow $+$7.9 pp)
      \end{itemize}
    \column{0.50\textwidth}
      \footnotesize
  ...
  \begin{columns}[T]
    ...
    \column{0.48\textwidth}
      ...
      \begin{itemize}
        ...
        \item Built an interactive website\\that allows to play and explore the agent in a real game environment
        \item Created a Monte Carlo tree search algorithm to estimate the true potential of any given game position.
